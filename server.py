...
async def api_mitre_upload(req: MitreUploadRequest):
...
async def api_risk_upload(req: RiskUploadRequest):
    filename = (req.filename or '').strip() or f'upload_{datetime.utcnow().isoformat()}'
    risk_save_dataset(
        filename=filename,
...
def filter_kri_dataset_by_year_quarter(dataset: Dict[str, Any], year: Optional[str], quarter: Optional[str]) -> Dict[str, Any]:
    """If year and quarter provided, filter; otherwise return dataset as-is."""
    if not year or not quarter:
        return dataset

    result: Dict[str, Any] = {}
    months = months_for_quarter(quarter)
        ...
        if 'applications' in content and content['applications'] is not None:
            ...
            for p in (content.get('data') or []):
                ...
                if period.startswith('Q'):
                    ...
                    # assume MM-YYYY (none in non-app KRIs right now)
                    mm, yy = period.split('-')
                    if yy == year and mm in months:
                        rows.append(p)
                ...
                for p in pts:
                    ...
                    for key in ['value', 'incidents', 'percentage', 'count']:
                        if key in p:
                            value = p[key]
                            break
                    html_parts.append(f'<tr><td>{esc(sys_name)}</td><td>{esc(period)}</td><td>{esc(str(value))}</td></tr>')
            ...
            for p in rows:
                ...
                for key in ['value', 'percentage', 'count']:
                    if key in p:
                        value = p[key]
                        break
                html_parts.append(f'<tr><td>{esc(period)}</td><td>{esc(str(value))}</td></tr>')
            html_parts.append('</tbody></table>')
    ...
    for rhi_id in sorted(filtered.keys()):
        ...
        for p in rows:
            ...
            if rhi_id == 'rhi107':
                ...
                val = f'{int(p.get("numerator",0))}/{int(p.get("denominator",0))} = {float(p.get("value",0.0)):.1f}%'
            else:
                val = str(p.get('value', ''))
